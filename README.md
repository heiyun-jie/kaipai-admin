...
默认本地开发地址为 `http://localhost:5100`，前端请求路径固定为相对路径 `/api`。
...
本地 `vite preview` 也保持同样的 `/api` 代理链，便于在 `5100 -> 127.0.0.1:8010` 这条本地联调路径下做人工验收。
线上构建不走这条本地代理链，而是由 nginx 统一承接站点与 `/api` 反代。
